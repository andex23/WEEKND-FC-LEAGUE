...
export default function ReportPage() {
  ...
  useEffect(() => {
    ;(async () => {
      try {
        ...
        const msgs = await fetch("/api/player/notifications")
          .then((r) => r.json())
          .catch(() => ({ messages: [] }))
        const rows = (fx.fixtures || []).map((f: any) => ({
          ...f,
          homeTeam: f.homeClub || f.homePlayer,
          awayTeam: f.awayClub || f.awayPlayer,
        }))
        setFixtures(rows)
  ...
  const selectedFixture = useMemo(
    () => eligible.find((f) => String(f.id) === String(selectedId)),
    [eligible, selectedId],
  )
  const youAreHome = useMemo(
    () => selectedFixture && user && selectedFixture.isHome === true,
    [selectedFixture, user],
  )
  const labels = useMemo(() => {
    if (!selectedFixture || !user) return { left: "You", right: "Opponent" }
    const youTeam = youAreHome
      ? selectedFixture.homeTeam || "You"
      : selectedFixture.awayTeam || "You"
    const oppTeam = youAreHome
      ? selectedFixture.awayTeam || "Opponent"
      : selectedFixture.homeTeam || "Opponent"
    ...
    try {
      ...
      if (file) {
        if (file.size > 2 * 1024 * 1024 || !["image/png", "image/jpeg"].includes(file.type))
          throw new Error("Use a PNG or JPEG screenshot smaller than 2 MB.")
        screenshotBase64 = await new Promise<string>((resolve, reject) => {
          const reader = new FileReader()
          reader.onload = () => resolve(String(reader.result))
          reader.onerror = () =>
            reject(new Error("Couldn't read your screenshot. Try another image."))
      ...
      setStatusMsg({
        type: result.status === "CONFLICT" ? "conflict" : "success",
        text: result.message || "Result submitted. Pending admin approval.",
      })
      setSelectedId("")
      setHomeScore("")
      setAwayScore("")
      ...
      setStatusMsg({
        type: "error",
        text: err.message || "Couldn't submit the result. Please try again.",
      })
    } finally {
      setSubmitting(false)
    }
  ...
  return (
    <div className="fc-site fc-form-page">
      <div className="container-5xl section-pad">
        <header className="mb-6">
          <PageHeading
            eyebrow="Player dashboard / match report"
            title="Report a result"
            description="Submit your match score for confirmation."
          />
          <p className="text-sm text-[#9E9E9E]">
            Submit your score with a screenshot. Results become official once approved by Admin.
          </p>
        </header>
                ...
                <>
                    ...
                    >
                      ...
                      {eligible.map((f) => {
                        ...
                        const opponent = isHome
                          ? f.awayTeam || f.awayPlayer
                          : f.homeTeam || f.homePlayer
                  ...
                  <div>
                    ...
                    <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs mb-1 text-[#9E9E9E]">
                  ...
                  {messages.slice(0, 5).map((m: any) => (
                    <li key={m.id} className="border-t first:border-t-0 border-[#1E1E1E] pt-3">
                      <div className="flex items-center gap-2 text-xs text-[#9E9E9E]">
                        <span className="inline-block rounded-full px-2 py-0.5 text-[10px] bg-emerald-600/15 text-emerald-400">
                          {m.read_at ? "Read" : "New"}
                        </span>
