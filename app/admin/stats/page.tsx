...
export default function AdminStatsPage() {
  ...
  const tournamentId = String(settings?.tournament?.active_tournament_id || "")
  ...
  const load = async () => {
    ...
    let fxRows: Row[] = []
    if (s?.tournament?.active_tournament_id) {
      const qs = `?tournamentId=${encodeURIComponent(String(s.tournament.active_tournament_id))}`
      ...
      try {
        ...
        const byIdLocal = new Map((playersRes.players || []).map((p: any) => [String(p.id), p]))
        const toRow = (r: any, field: "goals" | "assists") => ({ id: String(r.id || r.player_id || r.playerId || Math.random()), name: r.name || r.player_name || r.player || r.playerName, team: r.team || r.preferred_club || "-", [field === "goals" ? "G" : "A"]: r[field] || r[field === "goals" ? "G" : "A"] || 0 })
        setLeadersScorers((leaders.topScorers || []).map((r: any) => toRow(r, "goals")))
        setLeadersAssists((leaders.topAssists || []).map((r: any) => toRow(r, "assists")))
        setLeadersDiscipline((leaders.discipline || []).map((r: any) => ({ id: String(r.id || r.player_id || r.playerId || Math.random()), name: r.name, team: r.team || "-", YC: r.yellow_cards || r.YC || 0, RC: r.red_cards || r.RC || 0 })))
      } catch {
    ...
    <div className="min-h-screen bg-[#0D0D0D] text-white">
      <div className="container-5xl section-pad space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="text-right">
              <div className="text-xs text-[#9E9E9E]">Tournament</div>
              <div className="text-sm">{settings?.tournament?.name || "—"}</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <LeadersCard title="Top Scorers" table="scorers" fieldLabel="G" rows={leadersScorers} setRows={setLeadersScorers} addRow={addRow} deleteRow={deleteRow} overrideNumber={overrideNumber} updateMeta={updateMeta} onSave={publishStandings} />
          <LeadersCard title="Top Assists" table="assists" fieldLabel="A" rows={leadersAssists} setRows={setLeadersAssists} addRow={addRow} deleteRow={deleteRow} overrideNumber={overrideNumber} updateMeta={updateMeta} onSave={publishStandings} />
          <LeadersCard title="Discipline" table="discipline" fieldLabel="YC/RC" rows={leadersDiscipline} setRows={setLeadersDiscipline} addRow={addRow} deleteRow={deleteRow} overrideNumber={overrideNumber} updateMeta={updateMeta} isDiscipline onSave={publishStandings} />
        </div>
      </div>
    </div>
  )
...
function LeadersCard({ title, table, fieldLabel, rows, setRows, addRow, deleteRow, overrideNumber, updateMeta, isDiscipline, onSave }: { title: string; table: string; fieldLabel: string; rows: any[]; setRows: (v: any[]) => void; addRow: (t: string, n: string, tm?: string) => void; deleteRow: (t: string, id: string) => void; overrideNumber: (t: string, id: string, f: string, v: number) => void; updateMeta: (t: string, id: string, patch: { name?: string; team?: string }) => void; isDiscipline?: boolean; onSave: () => void }) {
  return (
    <div className="rounded-2xl border bg-[#141414] overflow-hidden">
      <div className="px-4 py-2 border-b border-[#1E1E1E] flex items-center justify-between">
        <div className="text-sm font-semibold">{title}</div>
        <div className="flex items-center gap-2">
          <Button size="sm" variant="outline" onClick={() => addRow(table, "", "")}>Add</Button>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-[#1E1E1E]">
                <td className="px-3 py-2"><Input className="bg-transparent h-8 w-full" value={r.name || ""} onChange={(e) => setRows(rows.map((x) => x.id === r.id ? { ...x, name: e.target.value } : x))} onBlur={() => updateMeta(table, r.id, { name: r.name })} placeholder="Player" /></td>
                <td className="px-3 py-2"><Input className="bg-transparent h-8 w-full" value={r.team || ""} onChange={(e) => setRows(rows.map((x) => x.id === r.id ? { ...x, team: e.target.value } : x))} onBlur={() => updateMeta(table, r.id, { team: r.team })} placeholder="Team" /></td>
                ...
                <td className="px-3 py-2 text-right"><Button size="sm" variant="outline" className="text-rose-400 border-rose-900 hover:bg-rose-900/20" onClick={() => deleteRow(table, r.id)}>Delete</Button></td>
              </tr>
            ))}
            {rows.length === 0 && (
