    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-black/60" onClick={() => setOpen(false)}>
          <div className="absolute left-0 top-0 bottom-0 w-72 bg-[#141414] border-r border-[#1E1E1E] p-4" onClick={(e) => e.stopPropagation()}>
            ...
            <nav className="space-y-1">
              {[
                ...
                <Link key={item.key} href={item.href} className="block px-3 py-2 rounded-md text-sm border bg-transparent hover:bg-[#0F0F0F] border-[#1E1E1E]" onClick={() => setOpen(false)}>
                  {item.label}
                </Link>
              ))}
            </nav>
